...
export interface ApiCallRecord {
  ...
  promptHash: string; // hash of the prompt content
  model: string;
  inputTokens: number;
  outputTokens: number;
  costUsd: number;
  responseChanged: boolean; // did the response differ from previous?
}
...
export interface PollingPattern {
  ...
  wasteRatio: number;  // fraction of calls that returned same result
  firstSeen: string;
  lastSeen: string;
}
...
export class HeartbeatOptimizer {
  ...
  detectPatterns(): PollingPattern[] {
    const patterns: PollingPattern[] = [];

    for (const [hash, calls] of this.callHistory) {
      if (calls.length < 3) continue; // need at least 3 calls to detect a pattern
  ...
  recommend(): OptimizationRecommendation[] {
    ...
    for (const pattern of patterns) {
      if (pattern.wasteRatio < 0.3) continue; // less than 30% waste, not worth optimizing
      ...
      let recommendedIntervalMs: number;
      let reason: string;
   ...
   */
  totalSavingsPotential(): { dailySavings: number; monthlySavings: number; patternCount: number } {
  ...
  private pruneOldRecords(hash: string): void {
