...
class SmartRouter {
    ...
    route(input) {
        ...
        const recommendedModelPricing = (0, model_pricing_js_1.getCheapestModel)(recommendedTier);
    ...
    analyzeComplexity(input) {
        ...
        for (const word of words) {
            if (SIMPLE_KEYWORDS.has(word))
                keywordScore -= 10;
        }
        // Check multi-word simple patterns
        for (const kw of SIMPLE_KEYWORDS) {
            if (kw.includes(' ') && prompt.includes(kw))
                keywordScore -= 15;
        }
        ...
        if (input.toolCount !== undefined) {
            if (input.toolCount === 0)
                toolScore = 10;
            else if (input.toolCount <= 2)
                toolScore = 40;
            else if (input.toolCount <= 5)
                toolScore = 60;
            else
                toolScore = 80;
        }
        ...
        let codeScore = 0;
        if (input.hasCodeContext)
            codeScore += 30;
        if (input.hasImages)
            codeScore += 20;
        // Check for code patterns in prompt
        if (/```[\s\S]*```/.test(input.prompt))
            codeScore += 25;
        if (/function\s|class\s|import\s|const\s|let\s|var\s/.test(input.prompt))
            codeScore += 15;
        codeScore = Math.min(100, codeScore);
        // Length scoring
        let lengthScore;
        if (promptTokenEstimate < 50)
            lengthScore = 10;
        else if (promptTokenEstimate < 200)
            lengthScore = 30;
        else if (promptTokenEstimate < 1000)
            lengthScore = 60;
        else
            lengthScore = 85;
        // Weighted total
        const totalScore = Math.round(keywordScore * 0.35 +
            lengthScore * 0.25 +
            toolScore * 0.20 +
            codeScore * 0.20);
        return {
    ...
    scoreToComplexity(score) {
        if (score <= this.complexityThresholds.simple)
            return 'simple';
        if (score >= this.complexityThresholds.complex)
            return 'complex';
        return 'medium';
    }
    calculateSavings(originalModel, recommended) {
        if (!originalModel || !recommended)
            return 0;
        // Approximate: compare input price ratios
        const originalPricing = (0, model_pricing_js_1.getCheapestModel)(originalModel.includes('opus') ? 'opus' : originalModel.includes('sonnet') ? 'sonnet' : 'haiku');
        if (!originalPricing)
            return 0;
        if (originalPricing.inputPerMillion === 0)
            return 0;
        return Math.max(0, 1 - (recommended.inputPerMillion / originalPricing.inputPerMillion));
    }
    buildReason(complexity, signals) {
        const parts = [];
        if (complexity === 'simple') {
            parts.push('Short prompt');
            if (signals.keywordScore < 30)
                parts.push('lookup/simple keywords detected');
            if (signals.toolScore <= 10)
                parts.push('no tool usage');
        }
        else if (complexity === 'complex') {
            if (signals.keywordScore > 70)
                parts.push('complex task keywords');
            if (signals.promptTokenEstimate > 500)
                parts.push(`long prompt (~${signals.promptTokenEstimate} tokens)`);
            if (signals.codeScore > 30)
                parts.push('code context present');
            if (signals.toolScore > 60)
                parts.push('multiple tools needed');
        }
        else {
