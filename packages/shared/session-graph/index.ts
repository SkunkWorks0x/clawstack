...
export class SessionGraph {
  ...
  constructor(dbPath?: string) {
    const resolvedPath = dbPath || join(process.cwd(), '.clawstack', 'session-graph.db');
  ...
  registerAgent(agent: Omit<AgentIdentity, 'agentId' | 'createdAt'> & { agentId?: string }): AgentIdentity {
  ...
  startSession(agentId: string, opts?: { parentSessionId?: string; pipelineId?: string; pipelineStep?: number }): AgentSession {
  ...
  endSession(sessionId: string, status: 'completed' | 'terminated' | 'error' = 'completed'): void {
    this.db.prepare(`
  ...
  getThreats(sessionId?: string, minLevel: ThreatLevel = 'low'): BehaviorEvent[] {
  ...
  recordCost(record: Omit<CostRecord, 'costId' | 'timestamp'>): CostRecord {
    ...
    `).run(
      ...
      record.routedBy,
      record.originalModel || null
    );

    return { costId, timestamp, ...record };
  }

  getSessionCost(sessionId: string): { tokens: number; costUsd: number; calls: number } {
  ...
  getAgentDailyCost(agentId: string, date?: string): { tokens: number; costUsd: number; calls: number } {
  ...
  getBudget(agentId: string): BudgetConfig | null {
    const row = this.db.prepare('SELECT * FROM budget_configs WHERE agent_id = ?').get(agentId) as any;
    if (!row) return null;
    return {
      agentId: row.agent_id,
  ...
  checkBudgetExceeded(agentId: string, sessionId: string): { session: boolean; daily: boolean; monthly: boolean } {
    const budget = this.getBudget(agentId);
    if (!budget) return { session: false, daily: false, monthly: false };
  ...
  queryMemory(agentId: string, workspace: string, tokenBudget: number): MemoryEntity[] {
    ...
    for (const row of rows) {
      if (tokensUsed + row.token_cost > tokenBudget) break;
  ...
  setSkillTrust(trust: SkillTrust): void {
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      trust.skillId, trust.skillName, trust.publisher, trust.trustLevel,
      trust.certifiedAt, trust.lastAuditAt, trust.threatHistory, trust.behavioralFingerprint
    );
  }

  getSkillTrust(skillId: string): SkillTrust | null {
    const row = this.db.prepare('SELECT * FROM skill_trust WHERE skill_id = ?').get(skillId) as any;
    if (!row) return null;
    return {
  ...
  getUntrustedSkills(): SkillTrust[] {
    const rows = this.db.prepare(
      "SELECT * FROM skill_trust WHERE trust_level IN ('unknown', 'untrusted') ORDER BY threat_history DESC"
    ).all() as any[];
    return rows.map(row => ({
      skillId: row.skill_id, skillName: row.skill_name, publisher: row.publisher,
      trustLevel: row.trust_level, certifiedAt: row.certified_at, lastAuditAt: row.last_audit_at,
  ...
  getSessionSummaries(limit = 50): any[] {
    return this.db.prepare('SELECT * FROM v_session_summary ORDER BY started_at DESC LIMIT ?').all(limit);
  }

  getAgentDailyCosts(agentId: string, days = 30): any[] {
    return this.db.prepare(
      'SELECT * FROM v_agent_daily_cost WHERE agent_id = ? ORDER BY day DESC LIMIT ?'
    ).all(agentId, days);
  }
  ...
  private mapSession(row: any): AgentSession {
    return {
  ...
  private mapBehaviorEvent(row: any): BehaviorEvent {
    return {
  ...
  private mapMemoryEntity(row: any): MemoryEntity {
    return {
