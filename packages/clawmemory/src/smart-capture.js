...
class SmartCapture {
        ...
        for (const rp of RELATION_PATTERNS) {
            ...
            while ((match = rp.pattern.exec(text)) !== null) {
                ...
                if (!sourceName || !targetName)
                    continue;
                if (isNoise(sourceName) || isNoise(targetName))
                    continue;
     ...
     */
    upsertEntity(extracted, agentId, workspace) {
    ...
    `).get(agentId, extracted.name, extracted.entityType, workspace);
        if (existing) {
            // Merge: update confidence (average weighted toward higher), update content if newer is more confident
            const newConfidence = Math.min((existing.confidence + extracted.confidence) / 2 + 0.05, 1.0);
     ...
     */
    upsertRelation(sourceEntityId, targetEntityId, rel) {
        const db = this.graph.getDb();
        const existing = db.prepare(`
      SELECT relation_id, weight FROM memory_relations
      WHERE source_entity_id = ? AND target_entity_id = ? AND relation_type = ?
    `).get(sourceEntityId, targetEntityId, rel.relationType);
        if (existing) {
     ...
     */
    extractContext(text, matchIndex, name) {
...
exports.SmartCapture = SmartCapture;
//# sourceMappingURL=smart-capture.js.map