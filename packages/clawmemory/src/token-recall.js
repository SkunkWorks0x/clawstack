 ...
 */
function computeRelevance(query, entity) {
    if (!query)
 ...
 */
function computeFrequency(accessCount) {
    if (accessCount <= 0)
        return 0;
...
class TokenRecall {
    ...
    async recall(options) {
        const { agentId, workspace, tokenBudget, query, preferredTypes, recencyWeight = 0.3, frequencyWeight = 0.2, confidenceWeight = 0.5, includeRelations = false, } = options;
    ...
    `).all(agentId, workspace);
        ...
        const scored = candidates.map(row => {
            ...
            const score = relevance * relevanceWeight +
    ...
    `);
        for (const entity of results) {
            ...
            // Emit access event
            await this.bus.emit((0, shared_1.createEvent)('memory.entity_accessed', 'clawmemory', {
      ...
      `).all(...Array.from(entityIds), ...Array.from(entityIds));
     ...
     */
    quickRecall(agentId, workspace, tokenBudget) {
...
exports.TokenRecall = TokenRecall;
//# sourceMappingURL=token-recall.js.map