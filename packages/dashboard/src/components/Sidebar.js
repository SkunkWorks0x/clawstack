...
export function Sidebar() {
    return (<aside className="w-56 h-screen bg-claw-surface border-r border-claw-border flex flex-col shrink-0">
      ...
      <nav className="flex-1 py-2 overflow-y-auto">
        {NAV_ITEMS.map(({ to, label, icon: Icon }) => (<NavLink key={to} to={to} end={to === '/'} className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${isActive
