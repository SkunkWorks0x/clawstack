...
export function Pipelines() {
    ...
    return (<div>
      ...
      {selectedId ? (<PipelineDetailView pipelineId={selectedId} onBack={() => setSelectedId(null)}/>) : (<PipelineList onSelect={setSelectedId}/>)}
    </div>);
}
function PipelineList({ onSelect }) {
    const { data, loading } = useApi(() => api.pipelines());
    if (loading)
    ...
    return (<div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((p) => (<tr key={p.pipelineId} className="table-row">
              ...
              <td className="py-2 pr-4">
                <button onClick={() => onSelect(p.pipelineId)} className="btn btn-ghost text-xs">
                  View
                </button>
              </td>
            </tr>))}
...
function PipelineDetailView({ pipelineId, onBack, }) {
    const { data, loading } = useApi(() => api.pipeline(pipelineId), [pipelineId]);
    if (loading)
        return <LoadingSpinner message="Loading pipeline..."/>;
    if (!data)
        return <EmptyState message="Pipeline not found."/>;
    ...
    return (<div>
      ...
      <div className="space-y-2">
        {steps.map((step, i) => (<div key={step.stepId} className="card flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {step.costUsd > 0 && (<span className="text-xs text-claw-muted">${step.costUsd.toFixed(4)}</span>)}
              </div>
              {step.result != null && (<pre className="text-xs text-claw-muted mt-2 bg-claw-bg rounded p-2 overflow-x-auto max-h-40">
      ...
      {/* YAML Definition */}
      {pipeline.definitionYaml && (<div className="mt-6">
