...
export function Security() {
    ...
    return (<div>
      ...
      <div className="flex gap-1 mb-6 border-b border-claw-border">
        {['threats', 'killswitch', 'trust'].map((t) => (<button key={t} onClick={() => setTab(t)} className={`px-4 py-2 text-sm border-b-2 transition-colors ${tab === t
...
function ThreatFeed() {
    const { data, loading } = useApi(() => api.threats('low'));
    if (loading)
        return <LoadingSpinner message="Loading threats..."/>;
    if (!data || data.length === 0) {
        return <EmptyState message="No threats detected. All clear."/>;
    }
    return (<div className="space-y-2">
      {data.map((event) => (<div key={event.eventId} className="card flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 text-sm">
              ...
              {event.threatSignature && (<span className="text-xs text-claw-muted">{event.threatSignature}</span>)}
            </div>
            <p className="text-xs text-claw-muted mt-0.5">
              Session {event.sessionId.slice(0, 8)} &middot; {formatTime(event.timestamp)}
            </p>
            {Object.keys(event.details).length > 0 && (<pre className="text-xs text-claw-muted mt-1 bg-claw-bg rounded p-2 overflow-x-auto">
...
function KillSwitchLog() {
    const { data, loading } = useApi(() => api.blockedEvents());
    if (loading)
        return <LoadingSpinner message="Loading kill switch log..."/>;
    if (!data || data.length === 0) {
    ...
    return (<div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((row) => (<tr key={row.event_id} className="table-row">
...
function SkillTrustTable() {
    ...
    if (loading)
        return <LoadingSpinner message="Loading skill trust..."/>;
    if (!data || data.length === 0) {
    ...
    return (<div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((skill) => (<tr key={skill.skill_id} className="table-row">
