...
type Tab = 'spending' | 'budgets' | 'router' | 'surgery';
...
function SpendingChart() {
  const { data, loading } = useApi<DailyCost[]>(() => api.dailyCosts(30));

  if (loading) return <LoadingSpinner message="Loading spending data..." />;
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <StatCard
          label="Avg/Day"
          value={`$${(totalSpent / Math.max(data.length, 1)).toFixed(2)}`}
          color="muted"
        />
      </div>
...
function BudgetTable() {
  const { data, loading } = useApi<BudgetStatus[]>(() => api.budgets());
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {data.map((b) => (
            <tr key={b.agentId} className="table-row">
                    ...
                    <div className="w-20 h-2 bg-claw-bg rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${
                          b.pctUsed > 90 ? 'bg-claw-danger' :
                          b.pctUsed > 70 ? 'bg-claw-warning' : 'bg-claw-accent'
                        }`}
...
function RouterStatsView() {
  const { data, loading } = useApi<RouterStats>(() => api.routerStats());

  if (loading) return <LoadingSpinner message="Loading router stats..." />;
  if (!data) return <EmptyState message="No routing data yet." />;
...
function SurgeryView() {
  const { data, loading } = useApi<SurgeryCandidate[]>(() => api.surgeryCandidates());

  if (loading) return <LoadingSpinner message="Analyzing contexts..." />;
  if (!data || data.length === 0) {
