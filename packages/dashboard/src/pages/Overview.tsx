...
export function Overview() {
  const { data, loading, error } = useApi<OverviewStats>(() => api.overview());
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          value={data.threatsBlockedToday}
          color={data.threatsBlockedToday > 0 ? 'danger' : 'success'}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard
          label="Spent Today"
          value={`$${data.moneySpentToday.toFixed(2)}`}
          color="warning"
        />
        <StatCard
          label="Saved by Smart Router"
          value={`$${data.moneySavedByRouter.toFixed(2)}`}
          sub="Estimated savings from model downgrading"
          color="success"
        />
        <StatCard
          label="Memory Entities"
          value={data.memoryEntities}
          sub="Across all agents and workspaces"
          color="accent"
        />
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {[
            ...
            <a
              key={link.label}
              href={link.href}
              className="text-sm text-claw-accent hover:text-claw-accent-bright transition-colors"
            >
              {link.label}
            </a>
          ))}
