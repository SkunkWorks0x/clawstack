...
export function Setup() {
  const { data, loading, refetch } = useApi<AgentSetup[]>(() => api.setupAgents());
          ...
          {data.map((agent) => (
            <div key={agent.agentId} className="card">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                ...
                <div>
                  {agent.dockerSandboxed ? (
                    <span className="text-claw-success">Enabled</span>
                  ) : (
                    <span className="text-claw-warning">Disabled</span>
                  )}
                </div>
