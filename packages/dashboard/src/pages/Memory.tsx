...
import { api, type MemoryEntity, type GraphData, type RecallResult, type Agent } from '../api';
import { LoadingSpinner } from '../components/LoadingSpinner';

type Tab = 'graph' | 'entities' | 'recall';
...
function EntityList() {
  ...
  const { data, loading, refetch } = useApi<MemoryEntity[]>(
    () => api.memoryEntities({
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState message="No memory entities found." />
      ) : (
...
function KnowledgeGraphView() {
  const { data, loading } = useApi<GraphData>(() => api.memoryGraph());
