...
export function createRoutes(graph: SessionGraph, bus: EventBus): Router {
  const router = Router();
  ...
  router.get('/overview', (_req: Request, res: Response) => {
    ...
    const threatsBlocked = (db.prepare(
      "SELECT COUNT(*) as c FROM behavior_events WHERE blocked = 1 AND date(timestamp) = ?"
    ).get(today) as any).c;

    const costToday = (db.prepare(
      'SELECT COALESCE(SUM(estimated_cost_usd), 0) as total FROM cost_records WHERE date(timestamp) = ?'
    ).get(today) as any).total;

    const routerSavings = (db.prepare(
      `SELECT COALESCE(SUM(estimated_cost_usd), 0) as saved FROM cost_records
       WHERE routed_by = 'smart_router' AND original_model IS NOT NULL AND date(timestamp) = ?`
    ).get(today) as any).saved;

    const memoryEntities = (db.prepare('SELECT COUNT(*) as c FROM memory_entities').get() as any).c;

    const activePipelines = (db.prepare(
      "SELECT COUNT(*) as c FROM pipelines WHERE status IN ('pending', 'running')"
    ).get() as any).c;
  ...
  router.get('/threats', (req: Request, res: Response) => {
    const minLevel = (req.query.minLevel as string) || 'low';
    const sessionId = req.query.sessionId as string | undefined;
    res.json(graph.getThreats(sessionId, minLevel as any));
  });

  router.get('/threats/blocked', (_req: Request, res: Response) => {
    const db = graph.getDb();
    const rows = db.prepare(
      `SELECT be.*, s.agent_id, a.name as agent_name
  ...
  router.get('/costs/today', (_req: Request, res: Response) => {
    const db = graph.getDb();
    const today = new Date().toISOString().split('T')[0];
    ...
    const row = db.prepare(
      `SELECT COALESCE(SUM(estimated_cost_usd), 0) as total,
              ...
              COUNT(*) as calls
       FROM cost_records WHERE date(timestamp) = ?`
    ).get(today);
  ...
  router.get('/costs/router-stats', (_req: Request, res: Response) => {
    const db = graph.getDb();
  ...
  router.get('/costs/surgery', (_req: Request, res: Response) => {
    const db = graph.getDb();

    // Find sessions with high token counts (potential bloated contexts)
    const rows = db.prepare(
      `SELECT s.session_id, s.agent_id, a.name as agent_name, s.status,
  ...
  router.get('/memory/entities', (req: Request, res: Response) => {
    const db = graph.getDb();
  ...
  router.get('/memory/graph', (req: Request, res: Response) => {
    const db = graph.getDb();
    ...
    if (!entityId) {
      ...
      res.json({
        nodes: (entities as any[]).map((e: any) => ({
          id: e.entity_id,
          name: e.name,
          type: e.entity_type,
          workspace: e.workspace,
        })),
        edges: (relations as any[]).map((r: any) => ({
    ...
    while (queue.length > 0) {
      ...
      const outgoing = db.prepare(
        'SELECT * FROM memory_relations WHERE source_entity_id = ?'
      ).all(current.id) as any[];

      const incoming = db.prepare(
        'SELECT * FROM memory_relations WHERE target_entity_id = ?'
      ).all(current.id) as any[];
    ...
    for (const nid of nodeIds) {
      const entity = db.prepare('SELECT * FROM memory_entities WHERE entity_id = ?').get(nid) as any;
      if (entity) {
        nodes.push({
          id: entity.entity_id,
          name: entity.name,
  ...
  router.get('/memory/recall', (req: Request, res: Response) => {
    const agentId = req.query.agentId as string;
    const workspace = req.query.workspace as string || 'default';
    const tokenBudget = parseInt(req.query.tokenBudget as string) || 2000;
  ...
  router.get('/setup/agents', (_req: Request, res: Response) => {
    const db = graph.getDb();
    const agents = graph.listAgents();

    const result = agents.map(agent => {
      const sessions = db.prepare(
        'SELECT COUNT(*) as total, SUM(CASE WHEN status = ? THEN 1 ELSE 0 END) as active FROM sessions WHERE agent_id = ?'
      ).get('active', agent.agentId) as any;
